...
bool ChannelStatsOp<CPUContext>::ComputeChannelStatsNCHW<float>(
    ...
    float* sum,
    float* sumsq) {
  ConstEigenArrayMap<float> X_arr(X, HxW, C);
  EigenVectorArrayMap<float> sum_arr(sum, C);
  EigenVectorArrayMap<float> sumsq_arr(sumsq, C);
  sum_arr = X_arr.colwise().sum();
  sumsq_arr = X_arr.square().colwise().sum();
  for (int i = 1; i < N; ++i) {
    ConstEigenArrayMap<float> Xi_arr(X + i * C * HxW, HxW, C);
    sum_arr += Xi_arr.colwise().sum();
    sumsq_arr += Xi_arr.square().colwise().sum();
  }
  return true;
}
